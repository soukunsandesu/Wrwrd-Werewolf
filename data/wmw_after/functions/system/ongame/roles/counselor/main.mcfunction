...
scoreboard players enable @a[scores={PrevRole=12,CurrentRole=200,counselor=0}] counselor

execute as @a if score @s PlayerNumber = @a[scores={CurrentRole=12},limit=1] counselor run tag @s add counselor
execute as @a if score @s PlayerNumber = @a[scores={PrevRole=12,CurrentRole=200},limit=1] counselor run tag @s add fakecounselor

# 本当のカウンセラー結果
execute if entity @a[tag=counselor] run tellraw @a[scores={counselor=1..}] [{"translate":" %sの陣営を反転させました。","with":[{"selector":"@a[tag=counselor]"}]}]
execute as @a[tag=counselor,scores={CurrentRole=1..200}] if entity @a[scores={counselor=1..,CurrentRole=12}] run function wmw_after:system/ongame/roles/counselor/switch_to_villager
execute as @a[tag=counselor,scores={CurrentRole=201..}] if entity @a[scores={counselor=1..,CurrentRole=12}] run function wmw_after:system/ongame/roles/counselor/switch_to_werewolf
